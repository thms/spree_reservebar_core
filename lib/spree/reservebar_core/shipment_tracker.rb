module Spree
  module ReservebarCore
    ...
    # Purpose: runs as a regular background job to get tracking information for all outstanding packages
    # and update shipment state.
    # Shipment is extended to send email notifications on shipment state changes
    class ShipmentTracker
      
      def self.get_all_shipment_events
        ShipmentTracker.get_all_fedex_events
        ShipmentTracker.get_all_ups_events
      end
      
      def self.get_all_fedex_events
        Spree::Retailer.active.each do |retailer|
          begin
            next if retailer.fedex_key.blank?
      ...
      def self.get_all_ups_events
        Spree::Retailer.active.each do |retailer|
          begin
            next if retailer.ups_key.blank?
            ShipmentTracker.get_ups_events_for_retailer(retailer)
          rescue Exception => e
            ShipmentTracker.log_exception(e)
          end
        end
      ...
      # retailers do not always click the order is ready for pick up button, which prevents the display state 
      #   from being correct, so we'll set order.packed_at to an hour ago if it is nil on the first transition.
      def self.get_ups_events_for_retailer(retailer)
        new_logger = Logger.new('log/shipment_tracker.log')
        new_logger.info("\n\n===== #{Time.now} =====")
        new_logger.info("===== #{retailer.name} start =====")

        ups = ActiveMerchant::Shipping::UPS.new(retailer.shipping_config)
        retailer.orders.complete.where(:shipment_state => ['pending', 'ready', 'shipped']).each do |order|
          new_logger.info("===== Retailer: #{retailer.name} Order #{order.number} old state: #{order.shipment_state} =====")
          order.shipments.each do |shipment|
            begin
              tracking_info = ups.find_tracking_info(shipment.tracking)
              # update shipment detail
              # Issues with UTF8 characters in the shipment events!
              begin
                shipment.shipment_detail.update_attribute_without_callbacks(:ship_events, tracking_info.shipment_events)
              ...
              new_state = ''
              case 
                 when shipment_state.include?("SCAN")
                   new_state = "shipped"
                 when shipment_state.include?("CUSTOMER")   
                   new_state = "shipped"
                 when shipment_state.include?("DELIVERY")   
                   new_state = "shipped"
                 when shipment_state.include?("DELIVERED")  
                   new_state = "delivered"
               end
